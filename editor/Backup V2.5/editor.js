...
// e.g. from a button
TeachingEngine.startTrack("architect-master-path");
...
if (RoleSystem.getRole() === "member") {
  TeachingEngine.startTrack("member-core-path");
}
...
document.getElementById("tw-open-training-portal")
  ...
  const trainingPortal = document.getElementById("tw-training-portal");
const trainingClose = document.getElementById("tw-training-close");
const trainingArchitectList = document.getElementById("tw-training-architect-list");
const trainingMemberList = document.getElementById("tw-training-member-list");
const openTrainingPortalBtn2 = document.getElementById("tw-open-training-portal") || document.getElementById("tw-open-training");

function renderTrainingPortal() {
  if (!trainingPortal) return;

  const prog = TeachingEngine.loadProgress();

  function renderTrackList(container, role) {
    if (!container) return;
    container.innerHTML = Object.entries(TeachingEngine.tracks)
      .filter(([_, t]) => t.role === role)
      .map(([id, t]) => {
        const completedCount = Object.keys(prog.completedSteps || {}).length; // simple for now
        return `
    ...
    container.querySelectorAll(".tw-training-start").forEach(btn => {
      btn.addEventListener("click", () => {
        ...
        alert(`Started: ${TeachingEngine.tracks[id].name}`);
      });
    });
  }

  renderTrackList(trainingArchitectList, "architect");
  renderTrackList(trainingMemberList, "member");
}

if (openTrainingPortalBtn2 && trainingPortal) {
  openTrainingPortalBtn2.addEventListener("click", () => {
    renderTrainingPortal();
    trainingPortal.style.display = "flex";
  });
}

if (trainingClose && trainingPortal) {
...
const checklist = document.getElementById("tw-training-checklist");
const checklistList = document.getElementById("tw-training-checklist-list");

function renderChecklist() {
  if (!checklist || !checklistList) return;
  ...
  checklistList.innerHTML = track.steps.map(step => {
...
// Call this whenever track/step changes
...
const _origNextStep = TeachingEngine.nextStep.bind(TeachingEngine);
...
// Patch TeachingEngine to support Training Mode
const _origStartTrack_TM = TeachingEngine.startTrack.bind(TeachingEngine);
TeachingEngine.startTrack = function(id) {
  if (!TRAINING_MODE) {
    alert("Training Mode is OFF. Turn it ON to start a track.");
    return;
  }
  _origStartTrack_TM(id);
  renderChecklist();
  applyHighlightForStep(this.currentTrack.steps[this.currentStepIndex]);
};

const _origNextStep_TM = TeachingEngine.nextStep.bind(TeachingEngine);
TeachingEngine.nextStep = function() {
  if (!TRAINING_MODE) return;
  _origNextStep_TM();
  renderChecklist();
...
if (trainingToggleBtn) {
  trainingToggleBtn.addEventListener("click", () => {
    ...
    if (TRAINING_MODE) {
      ...
      // If a track is active, show checklist + highlight
      renderChecklist();
...
// Optional: "Next Step" button in UI (if present)
const trainingNextBtn = document.getElementById("tw-training-next-step");

// Tooltip bubble for current step
...
// Extend existing highlight function to also show tooltip
...
// Auto-advance helpers (manual trigger for now)
function goToNextTrainingStep() {
  if (!TRAINING_MODE) return;
  if (!TeachingEngine.currentTrack) return;
  ...
  if (idx >= track.steps.length - 1) {
    // Last step completed
    alert(`Track complete: ${track.name}`);
    hideTrainingTooltip();
    clearHighlight();
    renderChecklist();
    return;
...
// Wire optional "Next Step" button
if (trainingNextBtn) {
...
// Hook: mark step as completed when we manually advance
const _origNextStep_ADV = TeachingEngine.nextStep.bind(TeachingEngine);
TeachingEngine.nextStep = function() {
  if (!TRAINING_MODE) return;
  ...
  if (step) {
    ...
    prog.completedSteps = prog.completedSteps || {};
    prog.completedSteps[step.id] = true;
    localStorage.setItem(this.storageKey, JSON.stringify(prog));
  }

  _origNextStep_ADV();

  // After moving to next step, update visuals
  renderChecklist();
  const newStep = this.currentTrack?.steps[this.currentStepIndex];
  applyHighlightForStep(newStep);
};

// Optional: auto-advance on certain actions (example hooks)
// You can expand these as you like.

// Example: when a block is added, advance if current step expects it
document.addEventListener("tw-block-added", () => {
  if (!TRAINING_MODE || !TeachingEngine.currentTrack) return;
  const step = TeachingEngine.currentTrack.steps[TeachingEngine.currentStepIndex];
  if (!step) return;
  if (step.id.includes("add-block") || step.id.includes("add-components")) {
    goToNextTrainingStep();
  }
});

// Example: when publish is opened, advance if step expects publish
const _origPublishClick_ADV = publishBtn && publishBtn.onclick;
if (publishBtn) {
  publishBtn.addEventListener("click", () => {
    if (typeof _origPublishClick_ADV === "function") {
      _origPublishClick_ADV();
    }
    if (!TRAINING_MODE || !TeachingEngine.currentTrack) return;
    const step = TeachingEngine.currentTrack.steps[TeachingEngine.currentStepIndex];
    if (!step) return;
    if (step.id.includes("publish") || step.id.includes("preview")) {
      goToNextTrainingStep();
    }
  });
}
...
function updateTrainingHUD() {
  if (!TRAINING_MODE || !TeachingEngine.currentTrack) {
  ...
  hudStep.textContent = `Step ${idx + 1} of ${track.steps.length}`;
...
// Patch startTrack to update HUD
const _origStartTrack_HUD = TeachingEngine.startTrack.bind(TeachingEngine);
TeachingEngine.startTrack = function(id) {
  if (!TRAINING_MODE) {
    alert("Training Mode is OFF. Turn it ON to start a track.");
    return;
  }
  _origStartTrack_HUD(id);
  updateTrainingHUD();
};

// Patch nextStep to update HUD
const _origNextStep_HUD = TeachingEngine.nextStep.bind(TeachingEngine);
TeachingEngine.nextStep = function() {
  if (!TRAINING_MODE) return;
  _origNextStep_HUD();
  updateTrainingHUD();
...
// When Training Mode toggles, update HUD
if (trainingToggleBtn) {
  trainingToggleBtn.addEventListener("click", () => {
    updateTrainingHUD();
  });
...
// =====================================================
// TRAINING MODE: AUTO-DETECT STEP COMPLETION
// =====================================================

// Helper: check if current step matches a keyword
function stepMatches(keyword) {
  if (!TRAINING_MODE || !TeachingEngine.currentTrack) return false;
  const step = TeachingEngine.currentTrack.steps[TeachingEngine.currentStepIndex];
  if (!step) return false;
  return step.id.includes(keyword) || step.title.toLowerCase().includes(keyword);
}
...
// Patch component add to dispatch event
const _origComponentAdd = COMPONENT_TEMPLATES["Hero"];
// Already wired in your editor.js — we add event dispatch:
function dispatchBlockAdded() {
  const evt = new Event("tw-block-added");
  document.dispatchEvent(evt);
}

// Patch canvas add logic
const _origAppendBlock = canvasBody.appendChild.bind(canvasBody);
canvasBody.appendChild = function(el) {
  const result = _origAppendBlock(el);
  dispatchBlockAdded();
  return result;
};

// =====================================================
// 2. Detect: Text Edited
// =====================================================
if (inspectorTitle) {
...
// =====================================================
// 3. Detect: Page Added or Switched
// =====================================================
if (addPageBtn) {
...
// =====================================================
// 6. Detect: Global Styles Changed
// =====================================================
...
// =====================================================
// 9. Detect: Identity Core Updated
// =====================================================
...
// =====================================================
// TRAINING MODE: ANIMATED ARROW POINTER
// =====================================================
...
function showTrainingArrowForStep(step) {
  ...
  const rect = el.getBoundingClientRect();
  const arrowSize = 40; // offset so arrow doesn't overlap element

  // Position arrow above the target element
...
// Patch highlight function to also position arrow
...
// Patch HUD update to reposition arrow
const _origUpdateHUD_ARROW = updateTrainingHUD;
updateTrainingHUD = function() {
  _origUpdateHUD_ARROW();
  if (!TRAINING_MODE || !TeachingEngine.currentTrack) {
    hideTrainingArrow();
    return;
  }
...
// Hide arrow when Training Mode toggles OFF
if (trainingToggleBtn) {
  trainingToggleBtn.addEventListener("click", () => {
    if (!TRAINING_MODE) hideTrainingArrow();
  });
}

// Hide arrow on window resize or scroll (then reposition)
...
// =====================================================
// TRAINING MODE: VOICE-GUIDED TRAINING
// =====================================================

// Voice settings from IdentityCore
...
function speakTraining(text) {
  ...
  // Optional: choose a voice that matches tone
...
// Patch startTrack to speak intro
...
TeachingEngine.startTrack = function(id) {
  ...
  const track = TeachingEngine.currentTrack;
  speakTraining(`Starting ${track.name}. Let's begin.`);
  speakCurrentStep();
};

// Patch nextStep to speak next step
...
TeachingEngine.nextStep = function() {
  ...
  if (idx >= track.steps.length) {
    speakTraining(`Congratulations. You have completed the ${track.name}.`);
    return;
  }
...
// Speak when Training Mode turns ON
if (trainingToggleBtn) {
  trainingToggleBtn.addEventListener("click", () => {
    if (TRAINING_MODE) {
...
// If stepMatches / goToNextTrainingStep not defined, define minimal versions
if (typeof stepMatches === "undefined") {
  window.stepMatches = function(keyword) {
    if (!TRAINING_MODE || !TeachingEngine.currentTrack) return false;
    const step = TeachingEngine.currentTrack.steps[TeachingEngine.currentStepIndex];
    if (!step) return false;
    return step.id.includes(keyword) || step.title.toLowerCase().includes(keyword);
...
if (typeof goToNextTrainingStep === "undefined") {
  window.goToNextTrainingStep = function() {
    if (!TRAINING_MODE || !TeachingEngine.currentTrack) return;
    TeachingEngine.nextStep();
  };
}
...
// =====================================================
// 1) VOICE-GUIDED MICRO-COACHING
// =====================================================

let trainingIdleTimer = null;
const TRAINING_IDLE_MS = 10000; // 10 seconds
...
function resetTrainingIdleTimer() {
  ...
  trainingIdleTimer = setTimeout(() => {
    ...
    // Simple micro-coaching lines based on step id/title
...
// Reset idle timer on common interactions
...
// Hook into step changes to reset timer
...
const _origStartTrack_MICRO = TeachingEngine.startTrack.bind(TeachingEngine);
TeachingEngine.startTrack = function(id) {
  if (!TRAINING_MODE) {
    alert("Training Mode is OFF. Turn it ON to start a track.");
    return;
  }
  _origStartTrack_MICRO(id);
...
// Wrap goToNextTrainingStep to celebrate on completion
...
// Optional: special celebrations for key events
...
// =====================================================
// 3) VOICE-GUIDED ARCHITECT CERTIFICATION EXAM
// =====================================================
...
const ExamEngine = {
  ...
  timerId: null,
  trackId: "architect-cert-exam-track",
  ...
  start() {
    ...
    TeachingEngine.startTrack(this.trackId);
...
// Example: register exam track (if not already registered)
if (!TeachingEngine.tracks["architect-cert-exam-track"]) {
  TeachingEngine.registerTrack("architect-cert-exam-track", {
    name: "Architect Certification Exam",
    role: "architect",
    steps: [
      { id: "exam-identity", title: "Set Identity", description: "Define archetype, voice, and promise.", target: "#tw-identity-panel", highlight: "pulse" },
      { id: "exam-portal", title: "Create Portal", description: "Create a new portal from template.", target: "#tw-new-portal-template", highlight: "pulse" },
      { id: "exam-pages", title: "Add Pages", description: "Add Home, About, and Services pages.", target: ".tw-page-list", highlight: "glow" },
      { id: "exam-components", title: "Add Components", description: "Add Hero, Feature Row, and Footer.", target: ".tw-component-list", highlight: "pulse" },
      { id: "exam-theme", title: "Style Theme", description: "Apply a cohesive theme in Global Styles.", target: "#tw-global-styles", highlight: "glow" },
      { id: "exam-export", title: "Export Site", description: "Export the site bundle.", target: "#tw-export-site", highlight: "pulse" },
      { id: "exam-publish", title: "Preview & Publish", description: "Open Publish and preview your work.", target: "#tw-publish-site", highlight: "glow" }
    ]
  });
}

// Hook: when exam track completes, stop exam
document.addEventListener("tw-architect-exam-complete", () => {
  if (ExamEngine.active) ExamEngine.stop(true);
});

// You can wire a button to start the exam:
// document.getElementById("tw-start-architect-exam")?.addEventListener("click", () => ExamEngine.start());

// =====================================================
// 4) VOICE-GUIDED MEMBER ONBOARDING
// =====================================================

const MEMBER_ONBOARD_TRACK_ID = "member-onboarding-voice-track";

if (!TeachingEngine.tracks[MEMBER_ONBOARD_TRACK_ID]) {
  TeachingEngine.registerTrack(MEMBER_ONBOARD_TRACK_ID, {
    name: "Member Onboarding",
    role: "member",
    steps: [
      {
...
// Auto-start onboarding for new members when Training Mode is ON
function maybeStartMemberOnboarding() {
  if (!TRAINING_MODE) return;
  if (RoleSystem?.getRole && RoleSystem.getRole() === "member") {
    speakTraining("Welcome. I'll gently guide you through your first edits.");
    TeachingEngine.startTrack(MEMBER_ONBOARD_TRACK_ID);
  }
}
...
// You can also call maybeStartMemberOnboarding() manually after role selection.
